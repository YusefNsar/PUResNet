...
def arg_parser():
    ...
    parser.add_argument('--mode', '-m', required=True, type=int,
                        help='Mode 0 is for single protein structure. Mode 1 is for multiple protein structure')
    parser.add_argument('--input_path', '-i', required=True, type=str,
                        help='For mode 0 provide absolute or relative path for protein structure. For mode 1 provide absolute or relative path for folder containing protein structure')
...
supported_formats = ['pdb', 'mol2']


def main():
    # print(pybel.outformats)
    os.chdir('/home/yusef/development/collage/PUResNet')
    ...
    if args.mode == 0:
        ...
        for name in os.listdir(args.input_path):
            ...
            o_path = os.path.join(
                args.output_path, os.path.basename(args.mol_path))
            if not os.path.exists(o_path):
                os.mkdir(o_path)
            model.save_pocket_mol2(mol, o_path, args.output_format)
